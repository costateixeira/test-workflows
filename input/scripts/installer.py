...
class installer(object):
    ...
    def load_multifile_schema(self) -> bool:
        ...
        try:
            with open(xsd_path, "rb") as xsd_file:
                ...
                self.logger.info(
                    f"Loaded multifile.xsd schema from {xsd_path}")
        except Exception as e:
            self.logger.info(
                f"FATAL: Could not load multifile.xsd from {xsd_path}: {e}")
    ...
    def process_multifile_xml(
            self, multifile_xml: Union[str, ET.Element]) -> bool:
        """
        ...
        try:
            ...
            if isinstance(multifile_xml, str):
                try:
                    ...
                    self.logger.info(
                        f"ERROR: Could not parse multifile_xml string: {e}")
                    return False
            ...
            if not file_elements:
                self.logger.info(
                    "WARNING: No <file> elements found in multifile XML.")
                return False
            for file_elem in file_elements:
                ...
                if not file_path:
                    self.logger.info(
                        "ERROR: <file> element missing 'name' attribute, skipping.")
                    continue
    ...
    def transform_xml(self,
        ...
        if prefix not in self.xslts:
            self.logger.info(
                "trying to transform unregistered thing " + prefix)
            return False
        if isinstance(xml, ET._ElementTree) or isinstance(xml, ET._Element):
            ...
            try:
                ...
                self.logger.info(
                    "ERROR: Generated invalid XML for " +
                    prefix +
                    "\n" +
                    f"\tError: {e}\n")
                return False
    ...
    def install_dmn(self, id, dmn: str):
        try:
            ...
            self.logger.info(
                "ERROR: Generated invalid XML for DMN id " +
                id +
                "\n" +
                f"\tError: {e}\n")
            return False
        ...
        for directory, instances in self.resources.items():
            for id, resource in instances.items():
                try:
                    ...
                    result = False
                    self.logger.info(
                        "Could not save resource of type: " +
                        directory +
                        "  with id: " +
                        id +
                        "\n")
