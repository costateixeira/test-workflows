...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="processes"
      select="/bpmn:definitions/bpmn:process[@name != '' and @id != ''] "
      />
  ...
  <xsl:variable
      name="questionnaires"
      select="/bpmn:definitions/bpmn:process/bpmn:userTask[@name != '' and @id != '']"/>
  ...
  <xsl:variable
      ...
      select="$questionnaires[ count(. | key('indexQuestId',@id)[1]) = 1]"/>  
