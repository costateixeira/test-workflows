...
"""
        ...
        for dt_id, dt_data in self.dt_data.items():
            ...
            # {dt_id}">View</a>,<a href="{dt_id}.dmn">DMN</a>)
            dt_desc += f"<br/>(<a href="
...
"""
    ...
    def extract_file(self):
        ...
        if (not self.extract_activities(cover_sheet)):
            self.logger.info(
                "Could not extract decision logic in: " +
                self.inputfile_name)
            return False
        self.logger.info(
            "Extracted COVER sheet in decision table: " +
            self.inputfile_name)
    ...
    def extract_activities(self, cover_sheet: pd.DataFrame):
        ...
        for index, row in cover_sheet.iterrows():
            ...
            if "id_name" in row and isinstance(
                    row["id_name"], str) and row["id_name"]:
                id_name = row["id_name"]
            ...
            if "dt_id" not in row or not isinstance(
                    row["dt_id"], str) or not row["dt_id"]:
            ...
            if not self.extract_activity_table(id, name, tab, dt_id, dt_data):
                self.logger.info(
                    "Could not extract decision table for id=" +
                    id +
                    " name=" +
                    name +
                    " data=" +
        ...
        for full_tab_id, dts in self.cql_definitions.items():
            ...
            for full_dt_id, cql_definitions in dts.items():
                ...
                for cql_id, val in cql_definitions.items():
                    if not cql_id:
                        self.logger.info(
                            "BAD:" + cql_id + " from " + full_dt_id)
                        sys.exit(2)
                    if cql_id not in all_codes:
                        self.logger.info(
                            "  adding new " + cql_id + " -> " + str(val))
                        ...
                        if title != all_codes[cql_id]['value']:
                            self.logger.info(
                                "ERROR: cql expression " +
                                cql_id +
                                " has repeated non-matching definition in decision table with id " +
                                full_dt_id +
                                " on tab_id " +
                                full_tab_id)
                ...
                dt_vs_title = 'Decision Table for ' + full_dt_id + \
                    ". Autogenerated from DAK artifacts"
                dt_vs = csm.render_vs_from_list(
                    dt_vs_id, self.installer.dd_prefix, dt_vs_title, dt_codes)
                if not dt_vs:
            ...
            self.create_cql_skeleton_for_tab(
                full_tab_id, tab_codes, properties)
            tab_vs_id = stringer.name_to_id(full_tab_id)
            csm = self.installer.get_codesystem_manager()
            tab_vs_title = 'Decision Tables For Tab ' + full_tab_id
            tab_vs = csm.render_vs_from_list(
                tab_vs_id,
                self.installer.dd_prefix,
                tab_vs_title,
                list(
                    tab_codes.keys()))
            if not tab_vs:
        ...
        for cql_id, cql_prop in all_codes.items():
            ...
            cql_prop['definition'] += " * Decision Tables: " + \
                ", ".join(cql_prop['table']) + "\n"
            cql_prop['definition'] += " * Tabs: " + \
                ", ".join(cql_prop['tab']) + "\n"
            if 'propertyString' not in cql_prop:
            ...
            for cql_file, cql_content in cql_contents.items():
                pattern = r'(^define\s+(\'|")' + re.escape(cql_id) + \
                    r'(\'|")\s*:(.*?))(\/\*|^define|\Z)'
                match = re.search(
                    pattern,
                    cql_content,
    ...
    def add_activities(self):
        for code, expr in self.cql_definitions_by_type['output'].items():
            ...
            self.installer.add_resource(
                'activitydefinitions', a_id, fsh_activity)

        return True

    def create_cql_skeleton_for_tab(self, tab_id, cql_codes, properties={}):
        lib_name = tab_id
        return self.installer.create_cql_library(
            lib_name, cql_codes, properties)
    ...
    def load_tab(self, tab: str):
        ...
        self.logger.info(
            "Attempting to load decision table metadata for tab=" + tab)
        try:
            df = pd.read_excel(
                self.inputfile_name,
                sheet_name=tab,
        ...
        for col in df:
            ...
            if not isinstance(matching_rows,
                              pd.Series) or not matching_rows.any():
                continue

            col_idx = df.columns.get_loc(col)
            self.logger.info(
                "Found decision tables in column index " +
                str(col_idx) +
                " on  matched rows=",
                df[matching_rows])
            for row in df[matching_rows].iterrows():
                row_idx = row[0]
                self.logger.info(
                    "Validating decision table on row= # " +
                    str(row_idx) +
                    "\n\t" +
                    '\t'.join(
                        str(x) for x in row[1].values))
                decision_id = stringer.name_to_id(row[1][col_idx + 1])
                if not isinstance(decision_id, str) or not decision_id:
                    self.logger.info(
                        "Could not find decision id to right of r,c:" + row + "," + col)
                    continue
                ...
                if not isinstance(
                        df[col][br_row],
                        str) or not df[col][br_row] == "Business rule":
                    self.logger.info(
                        "Did not find Business Rule row of decision table " +
                        decision_id)
                    continue
                br = df[col_idx + 1][br_row]
                if not isinstance(br, str) or not br:
                    self.logger.info(
                        "Did not find any Business Rule defined for decision table " +
                        decision_id)
                    continue
                ...
                if not isinstance(
                        df[col][trigger_row],
                        str) or not df[col][trigger_row] == "Trigger":
                    self.logger.info(
                        "Did not find trigger row of decision table " + decision_id)
                    continue
                trigger = df[col_idx + 1][trigger_row]
                if not isinstance(trigger, str) or not trigger:
                    self.logger.info(
                        "Did not find any trigger defined for decision table " +
                        decision_id)
                    continue

                input_row = row_idx + 3
                if not isinstance(
                        df[col][input_row], str) or (
                        not df[col][input_row] == "Inputs" and not df[col][input_row] == "Potential contraindications"):
                    self.logger.info(
                        "Did not find Inputs row of decision table " + decision_id)
                    continue
                self.logger.info(
                    "Found Inputs/Potential contraindications at " +
                    str(col) +
                    " / " +
                    str(input_row))
                ...
                if not output_col:
                    self.logger.info(
                        "Did not find Output column of decision table " + decision_id)
                    continue

                if not guidance_col:
                    self.logger.info(
                        "Did not find Guidance column of decision table " + decision_id)

                if not ref_col:
                    self.logger.info(
                        "Did not find Reference column of decision table " +
                        decision_id)
    ...
    def extract_activity_table(
            ...
            row):
        self.logger.info(
            "Looking for decision table ID=" +
            dt_id +
            " for activivity id /name (" +
            id +
            "/" +
            name +
            "): row=\n" +
        ...
        if dt_id not in self.tab_data[tab_id]['tables']:
            self.logger.info(
                ...
                " among found tables:" +
                ",".join(
                    self.tab_data[tab_id]['tables'].keys()))
            return False
        ...
        if stringer.name_to_id(
                ul_corner) == stringer.name_to_id("Decision ID"):
            ...
            self.logger.info(
                "Could not determine type of table from " +
                ul_corner +
                "/" +
                table_type)
            return False

        self.logger.info("Using decision tab of type (" +
                         table_type +
                         ") " +
                         dt_id +
                         " in sheet " +
        ...
        profile_id = stringer.name_to_id(
            self.installer.dd_prefix + "." + dt_id)
        full_dt_id = stringer.name_to_id(
            self.installer.dt_prefix + "." + dt_id)
        full_tab_id = stringer.name_to_id(
            self.installer.dt_prefix + "s." + tab_id)
        ...
        if is_contra_table:
            ...
            fsh['plan'] = self.get_fsh_plan(
                profile_id, full_tab_id, full_dt_id, full_lib_id, name)
        ...
        while in_table:
            ...
            if is_regular_table:
                ...
                if len(prev_rule['inputs']) == 0:
                    ...
                    dmn['input'].extend(
                        self.get_dmn_input_definition(
                            full_tab_id, full_dt_id, rule))
                else:
                    # it is a rule
                    dmn['rule'].extend(
                        self.get_dmn_input_rule(
                            full_tab_id, full_dt_id, row_offset, rule))
                    fsh['rules'] += self.get_fsh_rule(rule)
                    fsh['citations'] += self.get_fsh_citations(rule)
            elif is_contra_table:
                dmn['rule'].extend(
                    self.get_dmn_contraindication_rule(
                        full_tab_id, full_dt_id, row_offset, rule))
            else:
        ...
        if is_regular_table:
            fsh['plan'] += "\n" + fsh['citations'] + "\n" + fsh['rules']
            # maybe the DT was not in a bpmn, check to see if there is a
            # definition already loaded
            if not self.installer.has_resource('profiles', profile_id):
                self.logger.info(
                    "WARNING - Decision Table present without representation in BPMN")
                ...
                self.installer.add_resource(
                    'profiles', profile_id, profile_fsh)
            self.installer.add_resource(
                'plandefinitions', full_dt_id, fsh['plan'])
    ...
    def get_rule(self, df, data, row_offset, prev_rule):
        ...
        if t_row not in df[data["output_col"]]:
            return None
        ...
        trailing_blank_input = all([stringer.is_blank(v)
                                   for v in trailing_vals])
        ...
        blank_outputs = stringer.is_blank(
            rule['output']) and stringer.is_blank(
            rule['guidance']) and stringer.is_blank(
            rule['annotation'])

        if stringer.is_blank(
                first_val) and trailing_blank_input and blank_outputs:
            # end of table
            return None

        if len(
                prev_rule['inputs']) == 0 and not stringer.is_blank(
                prev_rule['annotation']):
        ...
        if (not stringer.is_blank(first_val)
            ) and trailing_blank_input and blank_outputs:
    ...
    def get_contra_dmns(self, dt_id, table_type):
        self.logger.info(
            "rendering contraindication input dmn for decision table ")
        contra_id = stringer.name_to_id(table_type + dt_id)
        contra_dmn_id = "input." + contra_id + "."
        contra_dmn = "<dmn:input id='" + contra_dmn_id + "' label='" + \
            stringer.xml_escape(table_type) + "'></dmn:input>"
        return [contra_dmn]
    ...
    def get_regular_dmns(self, dt_id):
        ...
        outputs += [
            self.create_dmn_output_expression(
                dt_id,
                output_name,
                output_expr)]
    ...
    def get_dmn_contraindication_rule(
            self,
            tab_id: str,
            dt_id: str,
            row_offset,
        ...
        for val in rule['inputs']:
            if stringer.is_blank(val) or stringer.is_dash(
                    val) or stringer.is_nan(val):
                continue
            self.logger.info("Adding contra '" + str(val) + "'")
            rule_dmn_entries.append(
                self.create_dmn_entry(
                    tab_id,
                    dt_id,
                    rule_name,
                    "input",
    ...
    def get_dmn_input_rule(self, tab_id: str, dt_id: str, row_offset, rule):
        ...
        for val in rule['inputs']:
            ...
            rule_dmn_entries.append(
                self.create_dmn_entry(
                    tab_id, dt_id, rule_name, "input", val))
    ...
    def get_dmn_input_definition(self, tab_id: str, dt_id: str, rule: dict):
        ...
        for val in rule['inputs']:
            ...
            if tab_id not in self.cql_definitions:
                self.cql_definitions[tab_id] = {}
            if dt_id not in self.cql_definitions[tab_id]:
                self.cql_definitions[tab_id][dt_id] = {}
            self.cql_definitions[tab_id][dt_id][val_id] = val_definition

            input_dmns.append(
                self.create_dmn_input_expression(
                    dt_id, val_id, val_definition))
            self.logger.info(
                "Added CQL with:\n\tTAB_ID=" +
                tab_id +
                "\n\tDT_ID=" +
    ...
    def get_fsh_plan(self, profile_id, tab_id, dt_id, lib_id, name):
        ...
        fsh_plan += 'Description: """' + \
            stringer.markdown_escape(name) + ' """\n'
    ...
    def get_output_activity_id(self, output_name):
        return stringer.name_to_id(
            self.installer.dt_prefix + "O." + output_name)
    ...
    def get_fsh_rule(self, rule: dict):
        ...
        if stringer.is_blank(rule['output']):
            self.logger.info(
                "ERROR: misformed decision table near rule=" +
                str(rule))
            sys.exit(44)
        ...
        if stringer.is_blank(rule['annotation']):
            ...
            # workaround for https://github.com/FHIR/sushi/issues/1569
            description = "\n" + rule['annotation'] + " "

        description = stringer.markdown_escape(
            rule['output'] + "\n" + description)
    ...
    def create_dmn_entry(
        ...
        if type == "input" or type == "output":
            ...
            if not (stringer.is_dash(name) or stringer.is_blank(
                    name) or stringer.is_nan(name)):
                if tab_id not in self.cql_definitions:
                    self.cql_definitions[tab_id] = {}
                if dt_id not in self.cql_definitions[tab_id]:
                    self.cql_definitions[tab_id][dt_id] = {}
        ...
        if expr:
            dmn += "<dmn:description>" + \
                stringer.xml_escape(expr) + "</dmn:description>"
    ...
    def create_dmn_output_expression(self, dt_id: str, name: str, expr: str):
        ...
        dmn = "<dmn:" + type + " id='" + dmn_id + \
            "' label='" + stringer.xml_escape(name) + "'>"
        if expr:
